<script setup>
  ...
  // 导入reportAPI  并命名空间（如 api）来访问所有的导出内容
  ...
  // 注册所有需要的组件和图表类型到 ECharts 中 只有注册过的模块才能在图表中使用
  echarts.use([
  ...
  const loadDegreeChart = async () => {
    const result = await api.queryStudentDegreeData()
    if(result.code){
      initDegreeChart(result.data)
      ElMessage.success("获取学历统计报表成功")
    }else{
  ...
  const loadStudentCountChart = async () => {
    const result = await api.queryStudentCountData()
    if(result.code) {
  ...
  function initStudentCountChart(clazzList, dataList) {
    ...
    myChart.setOption({
      ...
      xAxis: {
        data: clazzList // X 轴的数据来源是一个数组（clazzList），通常是班级名称列表。
      },
        ...
        series: [
          {
            ...
            data: dataList, // 数据来源是一个数组（dataList），通常是每个班级对应的人数。
      ...
      series: [ // 配置数据系列（图表的核心部分，包含实际的数据和样式）。
        {
          ...
          data: degreeDataList // 数据源，genderDataList 是一个数组，包含饼图的具体数据。
        }
      ]
    };
    // 绘制图表
